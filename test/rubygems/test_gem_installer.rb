...
class TestGemInstaller < Gem::InstallerTestCase
  ...
  def test_installer_regenerates_plugins
    installer = util_setup_installer do |spec|
      write_file File.join(@tempdir, "lib", "rubygems_plugin.rb") do |io|
        io.write "# do nothing"
      end
    ...
    # Directly exercise regenerate_plugins_for from InstallerUninstallerUtils
    installer.regenerate_plugins_for(installer.spec, Gem.plugindir)

    assert File.exist?(plugin_path), "plugin not regenerated by regenerate_plugins_for"
    assert_match %r{require_relative.*rubygems_plugin\.rb}, File.read(plugin_path),
                 "regenerated plugin should contain a require_relative to the plugin path"
  end

  def test_installer_handles_missing_plugins_dir
    installer = util_setup_installer do |spec|
      write_file File.join(@tempdir, "lib", "rubygems_plugin.rb") do |io|
        io.write "# do nothing"
      end
    ...
    refute File.directory?(plugins_dir), "plugins dir should not exist before regeneration"

    installer.generate_plugins

    assert File.directory?(plugins_dir), "plugins dir should be created by generate_plugins"

    plugin_path = File.join plugins_dir, "a_plugin.rb"
    assert File.exist?(plugin_path), "plugin should be regenerated even when plugins dir was missing"
  end

  def test_installer_regenerates_plugins_empty_plugins
    ...
    refute File.exist?(plugin_path), "no plugin file should exist for spec without plugins"

    installer.regenerate_plugins_for(installer.spec, Gem.plugindir)

    refute File.exist?(plugin_path), "regenerate_plugins_for should not create files for spec without plugins"
  end
